  ...
  <div class="app-container">
    ...
    <el-card class="box-card" v-loading="processInstanceLoading">
      ...
      <el-col v-if="this.processInstance.processDefinition && this.processInstance.processDefinition.formType === 10"
              :span="16" :offset="6">
        <div >
          <formRender :form-json="formJson" :form-data="formData" :option-data="optionData" ref="formRender">
          </formRender>
        </div>
      </el-col>
    ...
    getDetail() {
      ...
      getProcessInstance(this.id).then(response => {
        ...
        if (this.processInstance.processDefinition.formType === 10) {
          this.formJson = JSON.parse(this.processInstance.processDefinition.formConf)
          this.$nextTick(() => {
            this.$refs.formRender.disableForm(true)
            this.$refs.formRender.hiddenFormBtns(true)
          })
          // 设置表单的值
          this.formData = this.processInstance.formVariables
        }
      ...
      getTaskListByProcessInstanceId(this.id).then(response => {
        ...
        this.tasks.forEach(task => {
          ...
          if (!task.assigneeUser || task.assigneeUser.id !== user.id) { // 自己不是处理人
            return;
          }
