 ...
 *
 * 1. 异常提示信息，写在枚举类中，例如说，cn.iocoder.oceans.user.api.constants.ErrorCodeEnum 类 + BusinessExceptionConfiguration
...
public class BusinessExceptionUtil {
    ...
    public static BusinessException exception(IBaseErrorCodeEnum baseEnum) {
        String messagePattern = MESSAGES.getOrDefault(baseEnum.getCode(), baseEnum.getMsg());
        return exception0(baseEnum.getCode(), messagePattern);
    }

    public static BusinessException exception(IBaseErrorCodeEnum baseEnum, Object... params) {
        String messagePattern = MESSAGES.getOrDefault(baseEnum.getCode(), baseEnum.getMsg());
        return exception0(baseEnum.getCode(), messagePattern, params);
    }

    /**
     * 创建指定编号的 BusinessException 的异常
     *
     * @param code 编号
     * @return 异常
     */
